...
/** Minecraft 1.21 uses immutable stack components; every edit replaces CUSTOM_DATA through its update API. */
public final class BladeData {
    ...
    public record StoredEffect(Holder<MobEffect> effect, int amplifier, long expiresAt) {
    ...
    public static CompoundTag read(ItemStack stack) {
        return stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getCompound(ROOT);
    }
    ...
    public static boolean stealable(Holder<MobEffect> effect) { return !effect.value().isInstantenous() && effect.value().getCategory() == MobEffectCategory.BENEFICIAL; }
    public static Map<Holder<MobEffect>, StoredEffect> stolenEffects(ItemStack stack, long now) {
        Map<Holder<MobEffect>, StoredEffect> result = new LinkedHashMap<>();
    ...
    public static boolean capture(ItemStack sword, MobEffectInstance effect, long now) {
        ...
        for (StoredEffect stored : effects.values()) {
            ...
            entry.putString("Id", BuiltInRegistries.MOB_EFFECT.getKey(stored.effect().value()).toString());
    ...
    public static Map<Holder<MobEffect>, StoredEffect> desiredEffects(ItemStack sword, long now) {
        var effects = stolenEffects(sword, now);

...
当前版本 **0.4.1**。新增两把不可损坏的成长剑、灵魂吞噬、随机强化怪物和独立剑灵对话框。提供 **Fabric 1.20.1、Forge 1.20.1（Java 17）、Forge 1.12.2（Java 8）和 NeoForge 1.21.1（Java 21）** 四个版本。客户端和服务器均需安装对应版本。
...
- Fabric：安装 Fabric Loader 0.16.14+、Fabric API 0.92.2+1.20.1，将 `build/libs/thirteen-blade-0.4.1.jar` 放入实例的 `mods/`。
- NeoForge：安装 NeoForge 21.1.251+，将 `neoforge-1.21.1/build/libs/thirteen-blade-neoforge-1.21.1-0.4.1.jar` 放入 `mods/`。无需 Fabric API，详见 [该版本说明](neoforge-1.21.1/README.md)。
- Forge 1.20.1 / Better MC BMC4：使用 `forge-1.20.1/build/libs/thirteen-blade-forge-1.20.1-0.4.1.jar`，已按 Forge 47.4.20 构建，见 [版本说明](forge-1.20.1/README.md)。
- Forge 1.12.2 / RLCraft 2.9.3：使用 `forge-1.12.2/build/libs/thirteen-blade-forge-1.12.2-0.4.1.jar`，正式包在 Forge 14.23.5.2860 上验证，并适配 First Aid 分部位回血，见 [版本说明](forge-1.12.2/README.md)。
- 更新时移出旧版 JAR；不要安装 `-sources.jar`、`-dev.jar` 或带 `integration-tests` 的测试包。旧剑击杀数和能力继续保留，原蜘蛛「缓降」刻印自动按「隐身」生效。
- 基础剑：工作台中间一列从上到下为 **紫水晶碎片 → 下界合金剑 → 末影珍珠**。普通配方不继承原下界合金剑的附魔、名称和耐久。**Forge 1.12.2 特例：下界之星 → 钻石剑 → 末影珍珠**，因为旧版不存在前两种新版材料。
...
按键可在设置中修改；RLCraft / Better MC 存在相同快捷键时请重绑。整合包兼容测试范围见 [适配记录](docs/MODPACK_COMPATIBILITY.md)。主手、副手都支持技能、刻印和成长攻击，双手都有剑时主手优先；生命加成只要求剑在物品栏。对话窗口不暂停游戏。
...
- 快捷栏、普通背包格或副手中的剑提供生命，多把取最高值，不相加。不读取箱子、末影箱或潜影盒内部。仅携带或切换不会直接回血；有效击杀使成长等级提升时，先更新生命上限，再回满当时的全部生命（含其他来源的生命上限加成）。基础剑达到 10 级后不再升级回血。移除时截断超过新上限的当前生命。
...
两把剑固有 **抢夺 III** 和 **绑定诅咒 I**。合成取得、创造物品栏取出、命令获得以及旧剑进入玩家物品栏后都会自动补齐；不覆盖其他附魔，不降低已有的更高抢夺等级。固有附魔被移除后，服务器在持有时自动恢复。抢夺遵循原版主手武器的掉落规则。

绑定诅咒使用原版附魔语义：对手持剑不产生装备槽锁定，因此剑仍可移动、丢弃，死亡时仍按游戏规则掉落；它不是“灵魂绑定”或“死亡不掉落”。

每次有效击杀使剑实际提升成长等级，持有者都会恢复到当时的生命上限。普通击杀、收放剑、刷新属性和合成进阶本身不触发回血；基础剑封顶后仍记录击杀，但不再触发升级回血。此回血不补充伤害吸收的黄心。Forge 1.12.2 检测到 First Aid 1.6.x 时还会回满所有身体部位，上限仍服从整合包的 First Aid / Scaling Health 配置。
...
Fabric / Forge 1.20.1 原版没有旋风人，可自然获取其余 15 类刻印。Forge 1.12.2 可获取其中 12 类：没有幻翼、掠夺者、监守者和旋风人及相关新版效果；其他模组敌对物种仍可贡献韧性和持续正面药水效果。
...
构建输出为 `build/libs/thirteen-blade-0.4.1.jar`。项目固定使用 Gradle 8.7、Loom 1.6.12、Yarn 1.20.1+build.10、Loader 0.16.14、Fabric API 0.92.2+1.20.1。
